...
contract UniswapV3Factory is IUniswapV3PoolDeployer {
    ...
    function createPool(address tokenX, address tokenY, uint24 fee) public returns (address pool) {
        ...
        if (pools[tokenX][tokenY][fee] != address(0)) PoolAlreadyExists();
        parameters = PoolParameters({
            factory: address(this), token0: tokenX, token1: tokenY, tickSpacing: fees[fee], fee: fee});
